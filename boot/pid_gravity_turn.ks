...
UNTIL SHIP:APOAPSIS >= LAUNCH_CFG:targetApo {
    IF ALTITUDE > LAUNCH_CFG:turnStartAlt AND ALTITUDE < LAUNCH_CFG:turnEndAlt {
        IF SHIP:VELOCITY:SURFACE:MAG > 100 {
            ...
            PRINT "Alt: " + ROUND(ALTITUDE) + "m    " AT (0,4).
            PRINT "Current Pitch: " + ROUND(currentPitch,1) + "°   " AT (0,5).
            PRINT "Target Pitch: " + ROUND(targetPitch,1) + "°   " AT (0,6).
        }
...
UNTIL ABS(SHIP:ORBIT:APOAPSIS - SHIP:ORBIT:PERIAPSIS) < 1000 OR 
    ...
    PRINT "Vertical Vel: " + ROUND(verticalVel,2) + " m/s    " AT (0,10).
    PRINT "Pitch Adjust: " + ROUND(pitchAdjustment,2) + "°    " AT (0,11).
    PRINT "Apo: " + ROUND(SHIP:ORBIT:APOAPSIS/1000,1) + "km  Peri: " + ROUND(SHIP:ORBIT:PERIAPSIS/1000,1) + "km     " AT (0,12).
